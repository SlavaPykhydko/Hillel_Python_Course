...
year_search, engine_search, price_search = search_criteria
search_result = dict()
details = tuple()
first_n_results = 5
...
for car in car_data:
    # print(type(car_data[car]))
    color, year, engine, body, price =  car_data[car]
    if year_search <= year and engine_search <= engine and price_search >= price:
        details = (color, year, engine, body, price)
        # print(type(details))
        search_result[car] = details
        search_result.update({car: details})
# print(type(search_result))
search_result_after_slice = dict(islice(search_result.items(), first_n_results))
print(search_result_after_slice)
